...
with U1Nf2 only
"""
random_source(t0, corrws, u1ws::U1Nf2) = random_source(t0, corrws, [u1ws.params.am0, u1ws.params.am0], u1ws)

function random_source(t0, corrws, masses, u1ws::U1Nf2)
    if length(masses) != 2
        error("Only two masses allowed, but masses = $masses")
    end
    S0 = corrws.S0
    S = corrws.S
    R = corrws.R
    ...
    for ifl in 1:2
        ## Solve g5D R = S0 for S for Flavor ifl
        iter = LFTU1.invert!(S, LFTU1.gamm5Dw_sqr_msq_am0!(masses[ifl]), S0, u1ws.sws, u1ws)
        gamm5Dw!(R[ifl], S, masses[ifl], u1ws)
    end
    return nothing
end
export random_source
...
struct U1exCorrelator <: LFTU1.AbstractU1Correlator
    ...
    function U1exCorrelator(u1ws::LFTU1.U1; wdir::String = "./trash/", 
        ...
        gD = LFTU1.to_device(u1ws.device, SparseArrays.sparse([], [], complex(typeof(u1ws.U[1]))[], V, V))
        invgD1 = Nothing
        invgD2 = Nothing
        e1 = LFTU1.to_device(u1ws.device, zeros(complex(typeof(u1ws.U[1])), lp.iL..., 2))
        e2 = copy(e1)
        C = zeros(typeof(u1ws.U[1]), lp.iL[2])
        history = []
        mkpath(dirname(filepath))
